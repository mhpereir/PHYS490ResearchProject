    ...
    def plotResults(self):
        ...
        for i in range(0, 3):
            ...
            plot = axs[i, 0].scatter(
                target, resid, c=self.target_SN, s=s, cmap=cmap, vmin=50, vmax=250)
            axs[i, 0].plot(np.linspace(
                np.min(target)*0.9, np.max(target)*1.1, 10), np.zeros(10), 'k', linewidth=0.5)
        ...
        pplt.savefig(os.path.join(
            self.respath, self.trialname + '.png'), dpi=600)
